...
STR_FILE_OUT = "../Data/" + str(
    str(fre) + "GHz_" + str(int(angle)) + "deg_Hcut_" + today + ".txt"
)
arr2D_all_data = numpy.zeros(
    (nsamp, (4 * N_CHANNELS + 7))
)  # , where the 5 extra are f,x,y,phi, index_signal of peak cross power in a single bin (where phase is to be measured)
print(STR_FILE_OUT)
REGISTER_LO_1 = (
    5000
)  # Labjack register number for the Labjack DAC0 output, which goes to LO_1.
REGISTER_LO_2 = (
    5002
)  # Labjack register number for the Labjack DAC1 output, which goes to LO_2.
...
def get_data(baseline):
    ...
    for i in range(512):
        ...
        interleave_auto_a.append(
            a_0[i]
        )  # 'interleave' even and odd timestreams back into the original timestream (b.c. sampling rate is 2x your FPGA clock).
        interleave_auto_a.append(a_1[i])
        interleave_auto_b.append(
            b_0[i]
        )  # 'interleave' even and odd timestreams back into the original timestream (b.c. sampling rate is 2x your FPGA clock).
...
def TakeAvgData():
    ...
    arr_var_unique = numpy.zeros(
        (N_TO_AVG, 1)
    )  # A test to see how well we can use mean data over 'N_TO_AVG'.
    while j < N_TO_AVG:
        # print('In TakeAvgData(), j = ('+str(j)+'/N_TO_AVG)'+' and we are about to drawDataCallback')
        arr2D_aa[j], arr2D_bb[j], arr2D_ab[j], arr2D_phase[j], arr2D_index[
            j
