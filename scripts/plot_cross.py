...
def drawDataCallback(baseline):
    ...
    time.sleep(0.75)
    acc_n, interleave_cross_a, interleave_cross_b, interleave_auto_a, interleave_auto_b = fpga_daq.get_data(
        baseline, fpga
    )
    ...
    if (index_signal <= window / 2) and index_signal <= 2:
        ...
        max_mean_cross_pwr = np.max(
            (fpga_daq.running_mean(np.abs(interleave_cross_a), l_mean))[
                IGNORE_PEAKS_BELOW:IGNORE_PEAKS_ABOVE
            ]
        )
        max_mean_auto_A_pwr = np.max(
            (fpga_daq.running_mean(np.abs(interleave_auto_a), l_mean))[
                IGNORE_PEAKS_BELOW:IGNORE_PEAKS_ABOVE
            ]
        )
        max_mean_auto_B_pwr = np.max(
            (fpga_daq.running_mean(np.abs(interleave_auto_b), l_mean))[
                IGNORE_PEAKS_BELOW:IGNORE_PEAKS_ABOVE
            ]
        )
        index_mean_cross = (
